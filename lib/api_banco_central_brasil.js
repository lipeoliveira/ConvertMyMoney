...
const getCotacao = dependencias => async () => {
    try {
        const { getToday, getURL, getCotacaoAPI, extractCotacao } = dependencias 
...
module.exports = {
  ...
  getCotacaoAPI,
  extractCotacao,
  getCotacao: getCotacao({ getURL,  addZero,  getToday, getCotacaoAPI, extractCotacao, }),
  pure:{
    getCotacao
  }
}
